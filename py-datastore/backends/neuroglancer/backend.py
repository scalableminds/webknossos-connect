...
class NeuroglancerBackend(Backend):
  ...
  async def __handle_layer(self, layer_name, layer):
    ...
    layer['source'] = layer['source'][14:].replace('gs://', 'https://storage.googleapis.com/')
  ...
  async def handle_new_dataset(self, organization_name, dataset_name, dataset_info):
    layers = dict(await asyncio.gather(*(self.__handle_layer(*layer) for layer in dataset_info['layers'].items())))
  ...
  def __decode_jpeg(self, buffer, data_type, chunk_size):
    with BytesIO(buffer) as input:
      ...
      return np.asarray(list(image_data)).astype(data_type).reshape(chunk_size, order='F')
  ...
  def __chunks(self, offset, shape, scale, chunk_size):
    ...
    min_x = math.floor((min_x - scale.voxel_offset[0]) / chunk_size[0]) * chunk_size[0] + scale.voxel_offset[0]
    min_y = math.floor((min_y - scale.voxel_offset[1]) / chunk_size[1]) * chunk_size[1] + scale.voxel_offset[1]
    min_z = math.floor((min_z - scale.voxel_offset[2]) / chunk_size[2]) * chunk_size[2] + scale.voxel_offset[2]

    max_x = math.ceil((max_x - scale.voxel_offset[0]) / chunk_size[0]) * chunk_size[0] + scale.voxel_offset[0]
    max_y = math.ceil((max_y - scale.voxel_offset[1]) / chunk_size[1]) * chunk_size[1] + scale.voxel_offset[1]
    max_z = math.ceil((max_z - scale.voxel_offset[2]) / chunk_size[2]) * chunk_size[2] + scale.voxel_offset[2]
  ...
  async def __read_chunk(self, layer, scale, chunk_offset, chunk_size, decoder_fn):
    url_coords = '_'.join([f'{offset}-{offset + size}' for offset, size in zip(chunk_offset, chunk_size)])
    data_url = f'{layer.source}/{scale.key}/{url_coords}'

    response_buffer = await self.http_client.get(data_url, response_fn=lambda r: r.read())
  ...
  async def read_data(self, dataset, layer_name, resolution, offset, shape):
    ...
    chunks = await asyncio.gather(*(self.__read_chunk(layer, scale, chunk_offset, chunk_size, decoder) for chunk_offset, chunk_sizes in chunk_coords))

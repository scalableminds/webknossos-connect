...
class WebKnossosClient:
  ...
  async def report_status(self, ok=True):
    ...
    await self.http_client.patch(url, headers=self.headers, params=self.params, json=to_json(status))


  async def report_dataset(self, dataset):
    url = f'{self.webknossos_url}/api/datastores/{self.datastore_name}/datasource'
    headers = { 'Content-Type': 'application/json' }
    await self.http_client.put(url, headers=self.headers, params=self.params, json=to_json(dataset))
